...
- `PATCH /admin/businesses/:id` — the full-field editor: every profile field from §5's table (admin bypasses the tier gate — can set timeline on a basic-tier business), plus `business_tier`, `is_comped`, `hours` (same shape as §5's `PUT /businesses/:slug/hours` — omit to leave hours untouched, or send the full seven-day object to replace it; a partial day list is rejected), and a manual `latitude`/`longitude`/`neighborhood` override for addresses geocoding couldn't resolve.
